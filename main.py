...
class StoreApp(MDApp):
    ...
    def update_dashboard(self, date_str=None):
        ...
        for t in txs:
            if t['type'] == 'expense':
                icon, color, pre = "cash-minus", (0.8,0.2,0.2,1), "-"
            elif t['type'] == 'quick':
                icon, color, pre = "cash-plus", (0.2,0.6,0.2,1), "+"
            elif t['type'] == 'receipt':
            ...
            else: # Payment/Versement
                icon, color, pre = "cash-check", (0.2,0.2,0.6,1), "+"
            
            status_text = f" [{t.get('status', '')}]" if t['type'] == 'receipt' and t.get('status') != 'Paid' else ""
            li = TwoLineAvatarIconListItem(text=f"{t['desc']}{status_text}", secondary_text=f"{pre} {t['amount']:,.2f} | {t['time']}", on_release=lambda x, i=t: self.handle_click(i))
        ...
        if self.is_revenue_visible:
            lbl.text = f"{tot:,.2f} DZD"
            btn.icon = "eye"
        else:
            lbl.text = "****"
            btn.icon = "eye-off"
    ...
    # --- MANUAL PAYMENTS (DEBT PAGE) ---
    def open_manual_payment_dialog(self):
        content = Builder.load_string(KV_MANUAL_PAY)
        self.man_dialog = MDDialog(
            title="External Payment",
            type="custom",
            content_cls=content,
            buttons=[MDFlatButton(text="SAVE", on_release=self.save_manual_payment)]
        )
        self.man_dialog.open()

    def save_manual_payment(self, x):
        name = self.man_dialog.content_cls.ids.manual_client.text
        amt_txt = self.man_dialog.content_cls.ids.manual_amount.text
        if not name or not amt_txt: return
        
        try:
            amt = float(amt_txt)
            self.db.add_external_payment(name, amt)
            self.man_dialog.dismiss()
            pdf = self.generate_versement_pdf(name, amt)
            toast(f"Saved & PDF: {pdf}")
            self.update_dashboard()
        except: toast("Invalid Amount")

    def open_external_report(self):
        self.root.current = "ext_report"
        lst = self.root.get_screen('ext_report').ids.ext_list
        lst.clear_widgets()
        data = self.db.get_monthly_external_payments()
        if not data: lst.add_widget(OneLineListItem(text="No external payments this month"))
        
        for d in data:
            lst.add_widget(ThreeLineListItem(
                text=f"{d['client']}: {d['amount']:,.2f} DZD",
                secondary_text=f"{d['date']} | {d['time']}",
                tertiary_text=d['note']
            ))
    ...
    def open_new_receipt(self):
        ...
        s.ids.client_name.text = ""
        s.ids.amount_paid_input.text = "" 
        self.update_receipt_preview()
        self.root.current = "receipt_form"
    ...
    def add_to_cart(self):
        ...
        try:
            ...
            if not n: return
            t = p*q
            self.cart_items.append({"name":n, "price":p, "qty":q, "total":t})
            self.cart_total += t
            s.ids.cart_list.add_widget(OneLineListItem(text=f"{q}x {n} - {t:,.2f}"))
            s.ids.product_name.text=""; s.ids.product_price.text=""; s.ids.product_qty.text="1"
            self.update_receipt_preview()
        except: pass
    ...
    def update_receipt_preview(self, *args):
        s = self.root.get_screen('receipt_form')
        s.ids.preview_client.text = f"Client: {s.ids.client_name.text}"
        s.ids.preview_date.text = datetime.now().strftime("%Y-%m-%d %H:%M")
        txt = ""; 
        for i in self.cart_items: txt += f"{i['qty']}x {i['name']:<15} {i['total']:,.2f}\n"
        s.ids.preview_items.text = txt
        s.ids.preview_total.text = f"TOTAL: {self.cart_total:,.2f}"
        try: paid_now = float(s.ids.amount_paid_input.text)
        except: paid_now = 0.0
        if paid_now >= self.cart_total and self.cart_total > 0:
            s.ids.status_stamp.text = "PAID"; s.ids.status_stamp.text_color = (0, 0.8, 0, 1)
        elif paid_now > 0:
            s.ids.status_stamp.text = "PARTIAL"; s.ids.status_stamp.text_color = (1, 0.5, 0, 1)
        else:
            s.ids.status_stamp.text = "UNPAID"; s.ids.status_stamp.text_color = (0.8, 0, 0, 1)

    def save_receipt(self):
        if not self.cart_items: return
        s = self.root.get_screen('receipt_form')
        c = s.ids.client_name.text or "Unknown"
        items_txt = ", ".join([f"{i['qty']}x {i['name']}" for i in self.cart_items])
        try: paid_now = float(s.ids.amount_paid_input.text)
        except: paid_now = 0.0
        ...
        pdf = self.generate_pdf_receipt(c, status, paid_now)
        toast(f"Saved: {pdf}")
        self.update_dashboard()
        self.go_back()
    ...
    def generate_pdf_receipt(self, client_name, status, paid_now):
        ...
        for item in self.cart_items:
            c.drawString(10*mm, y, f"{item['qty']}x {item['name']}"); c.drawRightString(width - 10*mm, y, f"{item['total']:,.2f}"); y -= 5*mm
        ...
        c.drawString(10*mm, y, f"Remaining: {max(0, self.cart_total - paid_now):,.2f} DZD"); y -= 20*mm
        c.saveState(); c.translate(20*mm, y); c.rotate(15); c.setFont("Helvetica-Bold", 24)
    ...
    def open_credit_screen(self):
        ...
        lst = self.root.get_screen('credits').ids.credit_list
        lst.clear_widgets()
    ...
    def show_versement_dialog(self, receipt_id, remaining, client_name):
        self.temp_receipt_id = receipt_id
        self.temp_client_name = client_name
        content = Builder.load_string(KV_VERSEMENT)
        content.ids.debt_info.text = f"Owes: {remaining:,.2f} DZD"
        self.vers_dialog = MDDialog(title=f"Pay - {client_name}", type="custom", content_cls=content, buttons=[MDFlatButton(text="PAY", on_release=self.confirm_versement)])
        self.vers_dialog.open()

    def confirm_versement(self, x):
        try:
            amt = float(self.vers_dialog.content_cls.ids.versement_amount.text)
            self.db.add_versement(self.temp_receipt_id, amt)
            toast("Payment Added!")
            self.vers_dialog.dismiss()
            pdf = self.generate_versement_pdf(self.temp_client_name, amt)
            toast(f"PDF Generated: {pdf}")
            self.open_credit_screen(); self.update_dashboard()
        except: toast("Invalid Amount")
    ...
    def generate_versement_pdf(self, client, amount):
        ...
        c.setFont("Helvetica", 10); c.drawCentredString(width/2, height - 90*mm, "Thank you for your payment."); c.save()
        try: os.startfile(filename)
        except: pass
        return filename
    ...
    def confirm_expense(self, worker):
        note = self.exp_dialog.content_cls.ids.expense_note.text
        self.db.add_transaction(self.temp_expense_amount, True, worker, note)
    ...
    # --- REPORTS ---
    def open_report_screen(self): self.root.current="report"; self.update_report_list()
    def update_report_list(self):
        lst=self.root.get_screen('report').ids.report_list; lst.clear_widgets()
        for w,a in self.db.get_monthly_worker_expenses().items():
    ...
    def refresh_worker_list(self):
        ...
        for w in self.db.get_all_workers():
            li = OneLineAvatarIconListItem(text=w)
            li.add_widget(IconRightWidget(icon="trash-can", on_release=lambda x, n=w: self.remove_worker(n)))
            lst.add_widget(li)
    def add_worker(self):
    ...
    def toggle_revenue_privacy(self): self.show_pin_dialog() if not self.is_revenue_visible else self.update_dashboard()
    def show_pin_dialog(self): self.dialog = MDDialog(title="PIN", type="custom", content_cls=Builder.load_string(KV_PIN), buttons=[MDFlatButton(text="UNLOCK", on_release=self.check_pin)]); self.dialog.open()
    def check_pin(self,x):
        if self.dialog.content_cls.ids.pin_field.text==self.db.get_pin(): self.is_revenue_visible=True; self.update_dashboard(); self.dialog.dismiss(); self.dialog.content_cls.ids.pin_field.text=""
